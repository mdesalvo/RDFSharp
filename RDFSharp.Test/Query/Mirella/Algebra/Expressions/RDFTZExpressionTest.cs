...
public class RDFTZExpressionTest
{
    ...
    public void ShouldCreateTZExpressionWithExpression()
    {
        ...
        Assert.IsTrue(expression.ToString().Equals("(TZ((?V1 + ?V2)))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(TZ((?V1 + ?V2)))", System.StringComparison.Ordinal));
    }
    ...
    [TestMethod]
    public void ShouldThrowExceptionOnCreatingTZExpressionWithExpressionBecauseNullLeftArgument()
        => Assert.ThrowsExactly<RDFQueryException>(() => _ = new RDFTZExpression(null as RDFExpression));

    [TestMethod]
    public void ShouldThrowExceptionOnCreatingTZExpressionWithVariableBecauseNullLeftArgument()
    ...
    public void ShouldApplyExpressionWithExpressionAndCalculateResult()
    {
        ...
        Assert.IsTrue(expressionResult.Equals(new RDFTypedLiteral("Z", RDFModelEnums.RDFDatatypes.XSD_STRING)));
    }
