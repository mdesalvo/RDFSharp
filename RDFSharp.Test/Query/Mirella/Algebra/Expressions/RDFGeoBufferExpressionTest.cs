...
public class RDFGeoBufferExpressionTest
{
    ...
    public void ShouldCreateGEOBufferExpressionWithExpression()
    {
        ...
        Assert.IsTrue(expression.ToString().Equals($"(<{RDFVocabulary.GEOSPARQL.GEOF.BUFFER}>(?V, 150))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([RDFNamespaceRegister.GetByPrefix("geof")]).Equals("(geof:buffer(?V, 150))", System.StringComparison.Ordinal));
    }
    ...
    public void ShouldApplyExpressionWithVARAndCalculateResult()
    {
        ...
        table.AddRow(new Dictionary<string, string>
        {
            ...
            { "?MILAN", new RDFTypedLiteral("<gml:Point xmlns:gml=\"http://www.opengis.net/gml/3.2\"><gml:pos>9.18854 45.464664</gml:pos></gml:Point>", RDFModelEnums.RDFDatatypes.GEOSPARQL_GML).ToString() }
        });
