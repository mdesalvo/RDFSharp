...
public class RDFFloorExpressionTest
{
    ...
    public void ShouldCreateFloorExpressionWithExpression()
    {
        ...
        Assert.IsTrue(expression.ToString().Equals("(FLOOR((?V1 + ?V2)))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(FLOOR((?V1 + ?V2)))", System.StringComparison.Ordinal));
    }
    ...
    public void ShouldCreateFloorExpressionWithVariable()
    {
        ...
        Assert.IsNotNull(expression.LeftArgument);
        Assert.IsNull(expression.RightArgument);
        Assert.IsTrue(expression.ToString().Equals("(FLOOR(?V1))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(FLOOR(?V1))", System.StringComparison.Ordinal));
    }
