...
namespace RDFSharp.Test.Query
{
    ...
    public class RDFYearExpressionTest
    {
        ...
        public void ShouldCreateYearExpressionWithExpression()
        {
            ...
            Assert.IsTrue(expression.ToString().Equals("(YEAR((?V1 + ?V2)))"));
            Assert.IsTrue(expression.ToString(new List<RDFNamespace>()).Equals("(YEAR((?V1 + ?V2)))"));
        }
        ...
        public void ShouldCreateYearExpressionWithVariable()
        {
            ...
            Assert.IsNotNull(expression.LeftArgument);
            Assert.IsNull(expression.RightArgument);
            Assert.IsTrue(expression.ToString().Equals("(YEAR(?V1))"));
            Assert.IsTrue(expression.ToString(new List<RDFNamespace>()).Equals("(YEAR(?V1))"));
        }

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingYearExpressionWithExpressionBecauseNullLeftArgument()
            => Assert.ThrowsException<RDFQueryException>(() => new RDFYearExpression(null as RDFExpression));

        [TestMethod]
        public void ShouldThrowExceptionOnCreatingYearExpressionWithVariableBecauseNullLeftArgument()
            => Assert.ThrowsException<RDFQueryException>(() => new RDFYearExpression(null as RDFVariable));
