...
[TestClass]
public class RDFBooleanAndExpressionTest
{
    #region Tests
    [TestMethod]
    public void ShouldCreateBooleanAndExpression()
    {
        RDFBooleanAndExpression expression = new RDFBooleanAndExpression(
    ...
    public void ShouldThrowExceptionOnCreatingBooleanAndExpressionBecauseNullLeftArgument()
        => Assert.ThrowsExactly<RDFQueryException>(() => _ = new RDFBooleanAndExpression(null, new RDFVariableExpression(new RDFVariable("?V"))));

    [TestMethod]
    public void ShouldThrowExceptionOnCreatingBooleanAndExpressionBecauseNullRightArgument()
        => Assert.ThrowsExactly<RDFQueryException>(() => _ = new RDFBooleanAndExpression(new RDFVariableExpression(new RDFVariable("?V")), null));
    ...
    public void ShouldApplyExpressionAndCalculateResultTrue2()
    {
        ...
        table.AddRow(new Dictionary<string, string>
        {
            { "?A", new RDFTypedLiteral("5.1", RDFModelEnums.RDFDatatypes.XSD_DOUBLE).ToString() },
            { "?B", new RDFTypedLiteral("25", RDFModelEnums.RDFDatatypes.XSD_INT).ToString() },
            { "?C", RDFTypedLiteral.True.ToString() }
        });
    ...
    public void ShouldApplyExpressionAndCalculateResultFalse2()
    {
        ...
        table.AddRow(new Dictionary<string, string>
        {
            { "?A", new RDFTypedLiteral("5.1", RDFModelEnums.RDFDatatypes.XSD_DOUBLE).ToString() },
            { "?B", new RDFTypedLiteral("25", RDFModelEnums.RDFDatatypes.XSD_INT).ToString() },
            { "?C", RDFTypedLiteral.False.ToString() }
        });
