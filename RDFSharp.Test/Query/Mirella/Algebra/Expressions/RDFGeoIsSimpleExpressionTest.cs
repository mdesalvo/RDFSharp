...
public class RDFGeoIsSimpleExpressionTest
{
    ...
    public void ShouldCreateGEOIsSimpleExpressionWithExpression()
    {
        ...
        Assert.IsTrue(expression.ToString().Equals($"(<{RDFVocabulary.GEOSPARQL.IS_SIMPLE}>(?V))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([RDFNamespaceRegister.GetByPrefix("geosparql")]).Equals("(geosparql:isSimple(?V))", System.StringComparison.Ordinal));
    }
    ...
    public void ShouldThrowExceptionOnCreatingGEOIsSimpleExpressionWithEXPBecauseNullLeftArgument()
        => Assert.ThrowsExactly<RDFQueryException>(() => _ = new RDFGeoIsSimpleExpression(null as RDFVariableExpression));

    [TestMethod]
    public void ShouldThrowExceptionOnCreatingGEOIsSimpleExpressionWithVARBecauseNullLeftArgument()
        => Assert.ThrowsExactly<RDFQueryException>(() => _ = new RDFGeoIsSimpleExpression(null as RDFVariable));
    ...
    public void ShouldApplyExpressionWithVARAndCalculateResult()
    {
        ...
        table.AddRow(new Dictionary<string, string>()
        {
            { "?LS", new RDFTypedLiteral("LINESTRING(10.399658169597386 45.22267719962008,10.478759866207838 45.29537026093567,10.509828900381114 45.24408488739918,10.395538296550528 45.27488643704892)", RDFModelEnums.RDFDatatypes.GEOSPARQL_WKT).ToString() },
        });
