...
public class RDFIsNumericExpressionTest
{
    ...
    public void ShouldCreateIsNumericExpressionWithExpression()
    {
        ...
        Assert.IsTrue(expression.ToString().Equals("(ISNUMERIC((?V1 + ?V2)))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(ISNUMERIC((?V1 + ?V2)))", System.StringComparison.Ordinal));
    }
    ...
    public void ShouldCreateIsNumericExpressionWithVariable()
    {
        ...
        Assert.IsNotNull(expression.LeftArgument);
        Assert.IsNull(expression.RightArgument);
        Assert.IsTrue(expression.ToString().Equals("(ISNUMERIC(?V1))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(ISNUMERIC(?V1))", System.StringComparison.Ordinal));
    }
