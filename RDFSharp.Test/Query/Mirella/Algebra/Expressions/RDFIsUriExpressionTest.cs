...
[TestClass]
public class RDFIsUriExpressionTest
{
    #region Tests
    [TestMethod]
    public void ShouldCreateIsUriExpressionWithExpression()
    {
        RDFIsUriExpression expression = new RDFIsUriExpression(
            new RDFAddExpression(new RDFVariable("?V1"), new RDFVariable("?V2")));

        Assert.IsNotNull(expression);
        Assert.IsNotNull(expression.LeftArgument);
        Assert.IsNull(expression.RightArgument);
        Assert.IsTrue(expression.ToString().Equals("(ISURI((?V1 + ?V2)))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(ISURI((?V1 + ?V2)))", System.StringComparison.Ordinal));
    }

    [TestMethod]
    public void ShouldCreateIsUriExpressionWithVariable()
    {
        RDFIsUriExpression expression = new RDFIsUriExpression(
            new RDFVariable("?V1"));
        ...
        Assert.IsNotNull(expression.LeftArgument);
        Assert.IsNull(expression.RightArgument);
        Assert.IsTrue(expression.ToString().Equals("(ISURI(?V1))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(ISURI(?V1))", System.StringComparison.Ordinal));
    }

    [TestMethod]
    public void ShouldThrowExceptionOnCreatingIsUriExpressionWithExpressionBecauseNullLeftArgument()
        => Assert.ThrowsExactly<RDFQueryException>(() => _ = new RDFIsUriExpression(null as RDFExpression));

    [TestMethod]
    public void ShouldThrowExceptionOnCreatingIsUriExpressionWithVariableBecauseNullLeftArgument()
        => Assert.ThrowsExactly<RDFQueryException>(() => _ = new RDFIsUriExpression(null as RDFVariable));
