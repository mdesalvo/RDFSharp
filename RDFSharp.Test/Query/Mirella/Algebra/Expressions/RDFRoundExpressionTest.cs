...
public class RDFRoundExpressionTest
{
    ...
    public void ShouldCreateRoundExpressionWithExpression()
    {
        ...
        Assert.IsTrue(expression.ToString().Equals("(ROUND((?V1 + ?V2)))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(ROUND((?V1 + ?V2)))", System.StringComparison.Ordinal));
    }
    ...
    public void ShouldCreateRoundExpressionWithVariable()
    {
        ...
        Assert.IsNotNull(expression.LeftArgument);
        Assert.IsNull(expression.RightArgument);
        Assert.IsTrue(expression.ToString().Equals("(ROUND(?V1))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(ROUND(?V1))", System.StringComparison.Ordinal));
    }
    ...
    public void ShouldApplyExpressionWithVariableAndCalculateResult()
    {
        ...
        table.AddRow(new Dictionary<string, string>()
        {
            { "?A", new RDFTypedLiteral("11/4", RDFModelEnums.RDFDatatypes.OWL_RATIONAL).ToString() },
