...
public class RDFMonthExpressionTest
{
    ...
    public void ShouldCreateMonthExpressionWithExpression()
    {
        ...
        Assert.IsTrue(expression.ToString().Equals("(MONTH((?V1 + ?V2)))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(MONTH((?V1 + ?V2)))", System.StringComparison.Ordinal));
    }
    ...
    public void ShouldCreateMonthExpressionWithVariable()
    {
        ...
        Assert.IsNotNull(expression.LeftArgument);
        Assert.IsNull(expression.RightArgument);
        Assert.IsTrue(expression.ToString().Equals("(MONTH(?V1))", System.StringComparison.Ordinal));
        Assert.IsTrue(expression.ToString([]).Equals("(MONTH(?V1))", System.StringComparison.Ordinal));
    }
