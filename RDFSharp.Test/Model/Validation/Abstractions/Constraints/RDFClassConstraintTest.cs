...
public class RDFClassConstraintTest
{
    ...
    public void ShouldExportClassConstraint()
    {
        ...
        Assert.IsTrue(graph.Index.Hashes.Any(t => t.Value.SubjectID.Equals(new RDFResource("ex:NodeShape").PatternMemberID)
                                                    && t.Value.PredicateID.Equals(RDFVocabulary.SHACL.CLASS.PatternMemberID)
                                                    && t.Value.ObjectID.Equals(new RDFResource("ex:Class").PatternMemberID)));
    }
    ...
    public void ShouldNotConformNodeShapeWithPropertyShape()
    {
        ...
        RDFPropertyShape propShape = new RDFPropertyShape(new RDFResource("ex:PropShape"), RDFTestUtilities.ShaclPath(new RDFResource("ex:address")));
        ...
        Assert.IsTrue(validationReport.Results[0].ResultPath.AsSinglePredicate().Equals(new RDFResource("ex:address")));
        Assert.IsTrue(validationReport.Results[0].SourceConstraintComponent.Equals(RDFVocabulary.SHACL.CLASS_CONSTRAINT_COMPONENT));
