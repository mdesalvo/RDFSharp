...
public class RDFEqualsConstraintTest
{
    ...
    public void ShouldThrowExceptionOnCreatingEqualsConstraint()
        => Assert.ThrowsExactly<RDFModelException>(() => _ = new RDFEqualsConstraint(null));
    ...
    public void ShouldExportEqualsConstraint()
    {
        ...
        Assert.IsTrue(graph.Triples.AsEnumerable().Any(t => t.Field<long>("?SID").Equals(new RDFResource("ex:NodeShape").PatternMemberID)
                                                    && t.Field<long>("?PID").Equals(RDFVocabulary.SHACL.EQUALS.PatternMemberID)
    ...
    public void ShouldNotConformNodeShapeWithClassTarget()
    {
        ...
        Assert.AreEqual(4, validationReport.ResultsCount);
        ...
        Assert.AreEqual(RDFValidationEnums.RDFShapeSeverity.Violation, validationReport.Results[3].Severity);
        Assert.HasCount(1, validationReport.Results[3].ResultMessages);
