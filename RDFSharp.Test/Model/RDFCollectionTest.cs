...
public class RDFCollectionTest
{
    ...
    public void ShouldReifyCollectionWithOneItem(RDFModelEnums.RDFItemTypes itemType)
    {
        ...
        Assert.IsTrue(graph.GraphIndex.HashedTriples.ContainsKey(new RDFTriple(coll.ReificationSubject, RDFVocabulary.RDF.TYPE, RDFVocabulary.RDF.LIST).TripleID));

        switch (itemType)
        {
            case RDFModelEnums.RDFItemTypes.Literal:
                Assert.IsTrue(graph.GraphIndex.HashedTriples.ContainsKey(new RDFTriple(coll.ReificationSubject, RDFVocabulary.RDF.FIRST, new RDFPlainLiteral("lit1")).TripleID));
                break;
            case RDFModelEnums.RDFItemTypes.Resource:
                Assert.IsTrue(graph.GraphIndex.HashedTriples.ContainsKey(new RDFTriple(coll.ReificationSubject, RDFVocabulary.RDF.FIRST, new RDFResource("http://item1/")).TripleID));
                break;
        }
        Assert.IsTrue(graph.GraphIndex.HashedTriples.ContainsKey(new RDFTriple(coll.ReificationSubject, RDFVocabulary.RDF.REST, RDFVocabulary.RDF.NIL).TripleID));
    }
