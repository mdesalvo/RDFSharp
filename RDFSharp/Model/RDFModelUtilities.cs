...
namespace RDFSharp.Model
{
    ...
    public static class RDFModelUtilities
    {
        ...
        internal static (bool,string) ValidateTypedLiteral(string literalValue, RDFModelEnums.RDFDatatypes datatype)
        {
            ...
            switch (datatype)
            {
                ...
                case RDFModelEnums.RDFDatatypes.XSD_LANGUAGE:
                    return (RDFShims.LangTagRegex.Value.IsMatch(literalValue), literalValue);
                ...
                case RDFModelEnums.RDFDatatypes.XSD_HEXBINARY:
                    return (RDFShims.HexBinaryRegex.Value.IsMatch(literalValue), literalValue);
                #endregion
                ...
                case RDFModelEnums.RDFDatatypes.TIME_GENERALMONTH:
                    return (RDFShims.TimeGeneralMonthRegex.Value.IsMatch(literalValue), literalValue);

                case RDFModelEnums.RDFDatatypes.TIME_GENERALYEAR:
                    return (RDFShims.TimeGeneralYearRegex.Value.IsMatch(literalValue), literalValue);
                #endregion
                ...
                case RDFModelEnums.RDFDatatypes.OWL_RATIONAL:
                    return (RDFShims.OWLRationalRegex.Value.IsMatch(literalValue), literalValue);
        ...
        /// </summary>
        internal static RDFTypedLiteral ComputeNumericArithmetic(RDFTypedLiteral leftLiteral, RDFTypedLiteral rightLiteral, char arithmeticOperator)
        {
            //Ranks in the SPARQL numeric promotion lattice (integer < decimal < float < double); -1 = not numeric
            const int RANK_NONNUMERIC = -1;
            const int RANK_INTEGER    =  0;
            const int RANK_DECIMAL    =  1;
            const int RANK_FLOAT      =  2;
            const int RANK_DOUBLE     =  3;
            ...
            int GetNumericRank(RDFTypedLiteral typedLiteral)
            {
                ...
                {
                    case RDFModelEnums.RDFDatatypes.XSD_DECIMAL:
                    case RDFModelEnums.RDFDatatypes.OWL_RATIONAL: return RANK_DECIMAL;
                    case RDFModelEnums.RDFDatatypes.XSD_FLOAT:    return RANK_FLOAT;
                    case RDFModelEnums.RDFDatatypes.XSD_DOUBLE:
                    case RDFModelEnums.RDFDatatypes.OWL_REAL:     return RANK_DOUBLE;
                    default:                                      return RANK_INTEGER;   //integer family
                }
            }
