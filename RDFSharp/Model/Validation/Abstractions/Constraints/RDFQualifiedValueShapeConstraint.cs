...
{
    /// <summary>
    /// RDFQualifiedValueShapeConstraint represents a SHACL constraint requiring an exact minimum/maximum occurrency of the specified shape for a given RDF term
    /// </summary>
    public class RDFQualifiedValueShapeConstraint : RDFConstraint {
        ...
        internal override RDFValidationReport Evaluate(RDFValidationContext validationContext) {
            ...
            if (this.QualifiedValueMinCount.HasValue || this.QualifiedValueMaxCount.HasValue) {
                ...
                RDFValidationReport qualifiedValueShapeReport = new RDFValidationReport();
                if (qualifiedValueShape is RDFNodeShape)
                    qualifiedValueShapeReport.MergeResults(((RDFNodeShape)qualifiedValueShape).EvaluateNodeConstraintShape(qualifiedValueShapeValidationContext));
                else
                    qualifiedValueShapeReport.MergeResults(((RDFPropertyShape)qualifiedValueShape).EvaluatePropertyConstraintShape(qualifiedValueShapeValidationContext));
