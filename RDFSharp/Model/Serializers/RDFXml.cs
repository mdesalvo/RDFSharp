...
internal static class RDFXml
{
    ...
    internal static void Serialize(RDFGraph graph, Stream outputStream)
    {
        try
        {
            ...
            using (XmlTextWriter rdfxmlWriter = new XmlTextWriter(outputStream, RDFModelUtilities.UTF8_NoBOM))
            {
                ...
                var containers = graph.SelectTriples(p: RDFVocabulary.RDF.TYPE, o: RDFVocabulary.RDF.ALT)
                    .Union(graph.SelectTriples(p: RDFVocabulary.RDF.TYPE, o: RDFVocabulary.RDF.BAG))
                    .Union(graph.SelectTriples(p: RDFVocabulary.RDF.TYPE, o: RDFVocabulary.RDF.SEQ))
                    .Select(t => new
                    {
                ...
                var collections = graph.SelectTriples(p: RDFVocabulary.RDF.TYPE, o: RDFVocabulary.RDF.LIST)
                    .Select(t => new
                    {
                        CollectionUri = (RDFResource)t.Subject,
                        CollectionValue = graph.SelectTriples(s: (RDFResource)t.Subject, p: RDFVocabulary.RDF.FIRST).FirstOrDefault()?.Object,
                        CollectionNext = graph.SelectTriples(s: (RDFResource)t.Subject, p: RDFVocabulary.RDF.REST).FirstOrDefault()?.Object,
        ...
        XmlAttribute xmlLangPred, Dictionary<string, long> hashContext)
    {
        ...
        bool hasOneChildNode = predNode.HasChildNodes && predNode.ChildNodes.Count == 1;
        if (hasOneChildNode && predNode.ChildNodes[0]!.NodeType is XmlNodeType.Text or XmlNodeType.EntityReference)
        {
            RDFPlainLiteral pLit = new RDFPlainLiteral(RDFModelUtilities.ASCII_To_Unicode(HttpUtility.HtmlDecode(predNode.InnerText)), xmlLangPred?.Value);
