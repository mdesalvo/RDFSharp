...
namespace RDFSharp.Query
{
    ...
    public class RDFMaxAggregator : RDFAggregator
    {
        ...
        /// </summary>
        public RDFMaxAggregator(RDFVariable aggrVariable, RDFVariable projVariable, Boolean isDistinct, RDFQueryEnums.RDFMinMaxAggregatorFlavors aggregatorFlavor) : base(aggrVariable, projVariable, isDistinct)
        {
        ...
        private void ExecutePartitionString(String partitionKey, DataRow tableRow)
        {
            ...
            //Update aggregator context (max)
            this.AggregatorContext.UpdatePartitionKeyExecutionResult<String>(partitionKey, String.Compare(rowValue, aggregatorValue, true, CultureInfo.InvariantCulture) == 1 ? rowValue : aggregatorValue);
        }
