    ...
    public sealed class RDFStrLangExpression : RDFExpression
    {
        ...
        internal override RDFPatternMember ApplyExpression(DataRow row)
        {
            ...
            try
            {
                ...
                //We can only proceed if we have been given a well-formed language tag
                //(without direction, because it is supported by STRLANGDIR variant)
                if (rightArgumentPMember is RDFPlainLiteral rightArgumentPMemberLiteral
                     && RDFPlainLiteral.LangTagNoDirRegex.IsMatch(rightArgumentPMemberLiteral.Value))
                {
